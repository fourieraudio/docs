---
sidebar_position: -9999
---

# transform.suite '26

## Download

Ready to download the **transform**.suite '26 collection file?

First, please be sure to read the following EULAs from all third parties included in **transform**.suite '26.

<iframe src="https://appdata.fourieraudio.com/transform/Suite26EULA.html" height="500px" width="100%"></iframe>

:::info
By clicking the following button to download the **transform**.suite '26 collection file, you are agreeing to all terms as laid out in the above EULA.
:::
<a class="button button--lg button--primary" href="https://downloads.fourieraudio.com/transform/suite/25/fa-plugin-collection-transform.suite-26-bundle_26.0.0.collection">download **transform**.suite '26 collection file</a>  
[Not upgraded yet? Download transform.suite '25 (v1.1.0)](#transformsuite-25-version-110)



## Releases

| Suite Version | Release    | Minimum Engine Version | Release Date | Latest  |
| ------------- | ---------- | ---------------------- | ------------ | ------- |
| **'26**       | **26.0.0** | **1.6.0**              | 02/02/2026   | **Yes** |
| '25           | 1.1.0      | 1.5.0                  | 15/07/2025   |         |
| '25           | 1.0.1      | 1.3.0                  | 11/02/2025   |         |

:::tip

To upgrade the **transform**.suite, download the transform.suite collection file above, and follow the instructions listed in the [upgrade guide](/manual/transform.suite/upgrade-suite/).

:::
## Release Notes
### transform.suite '26 version: `26.0.0`

**NEW IN THIS RELEASE**
- Eventide Blackhole Immersive v1.4.4
- Sound Theory Gullfoss Live v1.11.6
- Slate Digital VMR independent modules:
  - Custom Opto
  - FG-36A
  - FG-A
  - FG-DS 902
  - FG-N
  - Gate: Classic
  - Gate: Drums
  - Revival
  - Transient Shaper
  - VTC London
  - VTC New York
  - VTC Hollywood

All plugins from **transform.suite ’25** remain included in **’26**, with the following updates and additions:

**Baby Audio**
- Crystalline v1.8 (Updated)
- Parallel Aggressor v1.5 (Updated)
- Spaced Out v1.7 (Updated)

**Eventide**
- Blackhole Immersive v1.4.4 (New)
- H3000 Factory MkII v1.3.4 (Updated)
- Shimmer Reverb v1.3.4 (Updated)
- SP2016 Reverb v1.8.5 (Updated)

**Harrison**
- 32 Classic Channel Strip v1.0.0.c89e7b41

**Plugin Alliance (NI / bx)**
- bx_aura v1.1.0
- bx_limiter True Peak v1.3.0 (Updated)
- bx_masterdesk classic v1.8.0 (Updated)
- bx_subsynth v1.9.1
- Shadow Hills Mastering Compressor v1.6.0 (Updated)
- SPL TransientDesigner Plus v1.10.1
- SPL TwinTube v1.18.1
- AMEK EQ200 v1.4.1
- bx_console AMEK 9099 v1.3.1

**Oeksound**
- soothe:live v1.0.3

**Slate Digital**
- Fresh Air v1.1.1
- Infinity Bass v1.0.7.0 (Updated)
- Infinity EQ 2 v2.0.7.1 (Updated)
- metaPitch v1.0.5.1 (Updated)
- metaTune v1.1.15.0 (Updated)
- MO-TT v1.1.13.1 (Updated)
- Stellar Echo v1.0.8.0 (Updated)
- VerbSuite Classics v1.1.11.0 (Updated)
- Virtual Tape Machines v1.2.10.2 (Updated)
- VMR FG-DS 902
- VMR FG-N
- VMR FG-A
- VMR Custom Opto
- VMR FG-36A
- VMR Revival
- VMR Gate: Classic
- VMR Gate: Drums
- VMR Transient Shaper
- VMR VTC London
- VMR VTC New York
- VMR VTC Hollywood

**sonible**
- true:level v1.0.4
- true:balance v1.0.5

**SSL**
- SSL 4K E v1.5.8 (Updated)
- SSL Bus Compressor 2 v1.8.9 (Updated)
- SSL Channel Strip 2 v2.9.11 (Updated)
- VocalStrip 2 v6.8.2 (Updated)
- LMC+ Drum Compressor v1.4.1 (Updated)
- DeEss v1.4.1 (Updated)
- Sourcerer v1.2.1 (Updated)
- X-Delay v1.4.4 (Updated)
- X-DynEQ v1.2.1 (Updated)
- X-Gate v1.2.2 (Updated)
- X-Limit v1.2.2 (Updated)
- X-Phase v6.9.3 (Updated)
- X-Saturator v6.9.3 (Updated)
- X-valvecomp v6.9.2 (Updated)

**tc electronic**
- VSS3 v2.1.0
- VSS4 HD v1.1.0
- LM2n v1.2.0
- NONLIN2 v1.1.0

**Universal Audio**
- Lexicon 224 Reverb v1.4.16 (Updated)
- Pultec EQP-1A EQ v1.2.16 (Updated)
- Pultec HLF-3C EQ v1.2.16 (Updated)
- Pultec MEQ-5 EQ v1.2.16 (Updated)
- Fairchild 660 Compressor v1.0.15 (Updated)
- Fairchild 670 Compressor v1.0.16 (Updated)
- dbx 160 v1.0.9 (Updated)
- 1176 Rev A Compressor v1.3.16 (Updated)
- 1176AE Compressor v1.3.16 (Updated)
- 1176LN Rev E Compressor v1.3.16 (Updated)
- LA-2 Compressor v1.3.16 (Updated)
- LA-2A Gray Compressor v1.3.16 (Updated)
- LA-2A Silver Compressor v1.3.16 (Updated)
- Empirical Labs EL8 Distressor v1.0.9 (Updated)
- LA-6176 v1.0.5 (Updated)

---

### transform.suite '25 version: `1.1.0`
<a class="button button--lg button--primary" href="https://downloads.fourieraudio.com/transform/suite/25/fa-plugin-collection-transform.suite-25-bundle_1.1.0.collection">download **transform**.suite '25 collection file</a>

**Eventide**
- H3000 Factory MkII v1.2.2

**Oeksound**
- soothe:live v1.0.3

**sonible**
- true:level v1.0.4
- true:balance v1.0.5

---

### transform.suite '25 version: `1.0.1`

We're very excited to release **transform**.suite '25, featuring:

**Baby Audio**
- Crystalline v1.7
- Parallel Aggressor v1.4
- Spaced Out v1.6

**Eventide**
- H3000 Factory MkII v1.2.0
- Shimmer Reverb v1.2.0
- SP2016 Reverb v1.7.0

**Harrison**
- 32 Classic Channel v1.0.0.c89e7b41

**Plugin Alliance**
- bx_aura v1.1.0
- Shadow Hills Mastering Compressor v1.5.1
- SPL TransientDesigner Plus v1.10.1
- bx_subsynth v1.9.1
- SPL TwinTube v1.18.1
- bx_masterdesk classic v1.7.1
- Amek EQ200 v1.4.1
- bx_console AMEK 9099 v1.3.1
- bx_limiter True Peak v1.2.1

**Oeksound**
- soothe:live v1.0.2

**Slate Digital**
- Fresh Air v1.1.1
- VMR - FG-DS 902 v2025.01.16-dbaf2bb115
- Virtual Tape Machines v1.2.9.1
- VMR - FG-N v2025.01.16-dbaf2bb115
- VMR - FG-A v2025.01.16-dbaf2bb115
- VMR - Custom Opto v2025.01.16-dbaf2bb115
- Infinity EQ 2 v2.0.2.1
- metaPitch v1.0.2.1
- MO-TT v1.1.10.1
- Stellar Echo SD-201 v1.0.3.1
- VMR - Gate:Classic v2025.01.16-dbaf2bb115
- VMR - Gate:Drums v2025.01.16-dbaf2bb115
- VMR - FG-36A v2025.01.16-dbaf2bb115
- VMR - Revival v2025.01.16-dbaf2bb115
- VMR - VTC London v2025.01.16-dbaf2bb115
- VMR - VTC New York v2025.01.16-dbaf2bb115
- VMR - VTC Hollywood v2025.01.16-dbaf2bb115
- VMR - Transient Shaper v2025.01.16-dbaf2bb115
- Infinity Bass v1.0.4.1
- metaTune v1.1.10.1
- VerbSuite Classics v1.1.9.1

**sonible**
- true:level v1.0.3
- true:balance v1.0.4

**SSL**
- X-Delay v1.3.1
- X-valvecomp v6.8.3
- LMC+ Drum Compressor v1.3.2
- SSL Bus Compressor 2 v1.6.2
- DeEss v1.3.2
- X-Limit v1.1.1
- X-Gate v1.1.1
- X-DynEQ v1.1.2
- X-Saturator v6.8.1
- 4K E v1.3.2
- Channel Strip 2 v2.7.3
- VocalStrip 2 v6.7.2
- X-Phase v6.8.1
- Sourcerer v1.1.2

**tc electronic**
- VSS3 v2.1.0
- VSS4 HD v1.1.0
- LM2n v1.2.0
- NONLIN2 v1.1.0

**Universal Audio**
- Lexicon 224 Reverb v1.4.12
- Pultec EQP-1A EQ v1.2.12
- Pultec HLF-3C EQ v1.2.12
- Pultec MEQ-5 EQ v1.2.12
- Fairchild 660 Compressor v1.0.11
- Fairchild 670 Compressor v1.0.12
- dbx 160 v1.0.5
- 1176 Rev A Compressor v1.3.12
- 1176AE Compressor v1.3.12
- 1176LN Rev E Compressor v1.3.12
- LA-2 Compressor v1.3.12
- LA-2A Gray Compressor v1.3.12
- LA-2A Silver Compressor v1.3.12
- Empirical Labs EL8 Distressor v1.0.5
- LA-6176 v1.0.1

---
